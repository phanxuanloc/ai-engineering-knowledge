...
3. `tutor` — runs the learning loop and proposes learning evidence.
...
6. `publisher` — converts validated knowledge/artifacts into durable shareable material.
...
Do not create a new agent when an existing agent can own the responsibility by using an additional skill.
...
- `learning-progress.yaml` when the task touches learning state
...
The human remains Principal/Owner. Agents may propose priorities, status changes, public publishing, architecture choices, and consulting recommendations, but must not fabricate approval or evidence.
