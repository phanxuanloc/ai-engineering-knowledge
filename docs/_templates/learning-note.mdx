...
Markdown first: chỉ dùng shared learning component, Mermaid, Docusaurus admonition
hoặc table khi chúng thực sự làm concept dễ hiểu và dễ scan hơn. Không bắt buộc
article phải dùng mọi component. Không dùng text code block thay cho diagram và
không tạo component mới nếu pattern hiện có đã đáp ứng được.
-->
...
Giải thích mental model đơn giản nhất để concept trở nên trực quan. Kết nối input, behavior và outcome. Dùng `ConceptGrid` + `ConceptCard` cho một nhóm nhỏ các vai trò liên quan; dùng `Pipeline` cho simple flow, pipeline hoặc linear lifecycle; chỉ dùng Mermaid cho branching, graph layout, sequence hoặc relationships phức tạp.
