...
Markdown first: dùng shared React/MDX learning và diagram primitives, Docusaurus
admonition hoặc table khi chúng thực sự làm concept dễ hiểu và dễ scan hơn. Không
bắt buộc article phải dùng mọi component. Không dùng Mermaid cho normal diagram,
không dùng text code block thay diagram và không tạo component riêng nếu các
primitive hiện có đã đáp ứng được. Mermaid chỉ là fallback cho graph thực sự lớn
và phức tạp, nơi automatic layout tạo ra giá trị đáng kể.
-->
...
Giải thích mental model đơn giản nhất để concept trở nên trực quan. Kết nối input, behavior và outcome. Dùng `ConceptGrid` + `ConceptCard` cho một nhóm nhỏ các vai trò liên quan; dùng `Pipeline` cho simple linear flow; compose `Diagram`, `Flow`, `Step`, `Group`, `Connection` và `Loop` cho các diagram khác.

<!-- Optional linear flow; remove when it does not improve understanding.
<Pipeline
  label="Concept pipeline"
  steps={[
    'Input',
    {label: 'Optional', title: 'Transform when useful', optional: true},
    {title: 'Outcome', description: 'Optional concise description.'},
  ]}
/>

Optional composed architecture:
<Diagram label="System architecture" variant="architecture">
  <Step title="System" />
  <Connection />
  <Flow>
    <Group title="Module A">
      <Step title="Responsibility 1" compact />
      <Step title="Responsibility 2" compact />
    </Group>
    <Group title="Module B">
      <Step title="Integration 1" compact />
    </Group>
  </Flow>
</Diagram>
-->
