...
Giải thích mental model đơn giản nhất để concept trở nên trực quan. Kết nối input, behavior và outcome. Dùng `ConceptGrid` + `ConceptCard` cho một nhóm nhỏ các vai trò liên quan; dùng `Pipeline` cho flow tuyến tính ngắn; dùng Mermaid cho architecture, lifecycle, sequence hoặc relationships phức tạp hơn.

<!-- Optional linear flow; remove when it does not improve understanding.
<Pipeline label="Concept pipeline" steps={['Input', 'Behavior', 'Outcome']} />
