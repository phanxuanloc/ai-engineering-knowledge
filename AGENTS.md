...
- Treat the learner's command or intention “Chốt kiến thức” as an invocation of the project-local `learning-checkpoint` skill at `.agents/skills/learning-checkpoint/SKILL.md`, not as a request for only a chat summary. Follow that workflow to capture or update the current learned knowledge and synchronize `learning-progress.yaml`. Capture only the material actually covered; preserve gaps and uncertainty, and do not treat the checkpoint as automatic completion of the whole broad topic.
...
- Run `.agents/skills/docs-ui/SKILL.md` before changing Docusaurus theme, navigation, article presentation, typography, learning-component styling, responsive behavior, or light/dark modes. `DESIGN.md` is the canonical visual specification. Prefer global semantic tokens and shared components; do not add article-scoped visual systems unless a unique teaching interaction requires one.
- Preserve the canonical shell across all docs: always-open independently scrollable sidebar, explicit domain → group → article hierarchy, major-section-only desktop TOC, readable prose width, high-contrast neutral light mode, deep navy dark mode, and cyan/teal as the single primary accent. Do not reintroduce purple primary accents, rainbow neon, washed-out light surfaces, or decorative dashboard chrome.
- Run a **visual-routing gate** before assigning or implementing any documentation task. Inspect the request and likely touched content for diagrams, flows, workflows, pipelines, architecture/topology, sequence/lifecycle, data movement, state transitions, animations, charts, or existing visual artifacts. When any signal is present, route the work through the existing role that owns the outcome and require that role to load `.agents/skills/docs-diagram/SKILL.md` before choosing a renderer-specific skill. Use Publisher / Knowledge Curator for the teaching model and canonical documentation integration, Builder for shared component or interactive visual implementation, and Reviewer for independent semantic and rendered-output acceptance; use Orchestrator when more than one role is required. Do not create a new role merely for a renderer, and do not let a general-purpose role bypass the relevant visual skill. During docs generation, this gate must produce an explicit visual-coverage decision for every new or substantially expanded learning note. If a visual would make the core mental model, structure, behavior, or quantitative relationship understandable materially faster than prose, creating it with the selected role and skill is required—not an optional polish step. A prose-only decision must pass the 3–5 second teaching test and state the concrete reason a visual adds no learning value; implementation convenience, schedule, or the presence of short prose are not valid reasons by themselves.
