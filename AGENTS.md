...
- Use `ExcalidrawDiagram` from `src/components/Diagrams` to render optimized SVG exports. Keep reading pages light by not shipping the Excalidraw editor. Treat the `.excalidraw` JSON as canonical source, keep it beside its SVG under `static/img/diagrams/excalidraw/`, and run `npm run diagrams:build` after source changes. Never hand-edit a generated SVG independently of its scene. Supply meaningful `alt` text and a concise caption.
