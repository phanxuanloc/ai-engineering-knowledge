...
- Treat published React Flow as a static illustration, never an editor canvas. Source/target handles may exist only for edge calculation and must be visually hidden. Disable dragging, selection, connection creation, keyboard manipulation, pan/zoom UI, minimaps, controls, and selection affordances unless a specific interaction teaches the concept.
