...
- Reuse components from `src/components/Learning`: `TLDR` for the required summary, `Principle` for memorable invariants or decision rules, `ConceptCard` with `ConceptGrid` for small related mental-model groups, `Comparison` when comparison is the primary teaching structure, and `Pipeline` for simple flows, pipelines, and linear lifecycles. `Pipeline` supports optional labels, descriptions, optional stages, and a loop caption. Import only the components a note uses.
- Use semantic HTML/CSS learning components—not Mermaid—by default for simple conceptual flows, pipelines, lifecycle diagrams, comparisons, and mental models. They provide consistent layout, responsive behavior, accessibility, and light/dark styling.
- Keep Mermaid available for diagrams that genuinely need graph layout: branching architecture, complex relationships, sequence diagrams, or quickly authored technical graphs. Do not use plain-text code blocks as diagrams or use Mermaid as decoration.
